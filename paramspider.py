...
def main():
    ...
    # Our for Natrix and getting json:
    parser.add_argument('-f', '--output-file', help='Output file name json')
    ...
    if response:
        ...
        # result_dict["amount_urls"] = len(final_uris)

        if final_uris:
            result_dict["urls"] = []
            for url in final_uris:
                if url != "FUZZ":
                    result_dict["urls"].append({"url": url})
